...
void shadeLightPoints(LED myLED, ArrayList<lightPoint> newLightPoints)
{
  ...
  for(int j = 0; j < newLightPoints.size(); j++)
  {
    ...
    float dist = dist(myLED.tesLoc.x, myLED.tesLoc.y, myLED.tesLoc.z, lightSource.location.x, lightSource.location.y, lightSource.location.z);
...
/*
void shadeWithinPoint(LED myLED, ArrayList<lightPoint> newLightPoints)
{
  float dist = dist(myLED.tesLoc.x, myLED.tesLoc.y, myLED.tesLoc.z, newLightPoint.location.x, newLightPoint.location.y, newLightPoint.location.z);
...
*/
void shadeWithinBlocks(LED myLED, ArrayList<lightBlock> newLightBlocks)
{
  for(int i = 0; i < newLightBlocks.size(); i++)
  {
    lightBlock newLightBlock = newLightBlocks.get(i);
    if( newLightBlock.isThisInside(myLED.tesLoc) )
    {
...
void suggestLightBlocks(LED myLED, ArrayList<lightBlock> newLightBlocks)
{
  boolean lightCheck = false;
  for(int i = 0; i < newLightBlocks.size(); i++)
  {
    if(newLightBlocks.get(i).isThisInside(myLED.tesLoc))
    {
      lightCheck = true;
    }
  }
